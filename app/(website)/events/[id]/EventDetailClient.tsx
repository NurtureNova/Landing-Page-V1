...
import { Calendar, MapPin, Clock, ArrowLeft, ArrowRight, Share2, ShieldCheck, Users } from 'lucide-react';
    ...
    return (
        <div className="grid grid-cols-4 gap-2 text-center">
            {[
                ...
                <div key={unit.label} className="bg-white rounded-xl p-2 border border-gray-100 shadow-sm">
                    <div className="text-xl font-black text-blue-600 leading-none mb-1">{unit.val}</div>
                    <div className="text-[8px] uppercase font-bold text-gray-400 tracking-widest">{unit.label}</div>
                </div>
            ))}
        </div>
    );
...
export default function EventDetailClient({ id }: { id: string }) {
    ...
    if (loading) return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-white">
            ...
            <p className="mt-4 text-gray-400 font-bold animate-pulse uppercase tracking-widest text-xs">Loading Event...</p>
        </div>
    );
    ...
    if (!event) return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-white px-4">
            ...
            <h2 className="text-3xl font-black text-gray-900 mb-2">Event Not Found</h2>
            <p className="text-gray-500 mb-8 text-center max-w-xs font-medium">The event you&apos;re looking for might have been removed or is currently unavailable.</p>
            <Link href="/events" className="px-8 py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all">
                Back to All Events
            </Link>
        </div>
    );
    ...
    const isCapacityReached = event.maxApplicants && event.applicationCount !== undefined && event.applicationCount >= event.maxApplicants;
        ...
        <div className="min-h-screen bg-white">
            <main className="pt-32 pb-24">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <Link href="/events" className="inline-flex items-center text-gray-400 hover:text-blue-600 mb-10 font-bold transition-colors group">
                        ...
                        <div className="lg:col-span-8 overflow-hidden">
                            ...
                            >
                                <div className="space-y-4">
                                    ...
                                    <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black text-gray-900 leading-tight tracking-tight break-words">
                                        {event.title}
                                    </h1>
                                </div>

                                <div className="relative aspect-video rounded-[2rem] overflow-hidden bg-gray-100 border border-gray-200 shadow-xl shadow-gray-200/50">
                                    {event.imageUrl ? (
                                ...
                                <div className="space-y-6 pt-4">
                                    ...
                                    <div 
                                        className="prose prose-blue prose-img:rounded-3xl max-w-none text-gray-600 leading-relaxed font-medium break-words overflow-hidden"
                                ...
                                <div className="bg-white rounded-[2rem] border border-gray-200 p-8 space-y-8 shadow-sm">
                                    {event.status !== 'Completed' && (
                                        <div className="space-y-4 pb-8 border-b border-gray-100">
                                            <p className="text-[10px] text-center text-gray-400 font-black uppercase tracking-widest">Starts In</p>
                                    ...
                                    <div className="space-y-6">
                                        <div className="flex items-start">
                                            ...
                                            <div className="overflow-hidden">
                                                <p className="text-[9px] text-gray-400 font-black uppercase tracking-widest mb-0.5">Date</p>
                                                <p className="text-gray-900 font-bold text-sm break-words">{eventDate.toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-start">
                                            ...
                                            <div className="overflow-hidden">
                                                <p className="text-[9px] text-gray-400 font-black uppercase tracking-widest mb-0.5">Time</p>
                                                <p className="text-gray-900 font-bold text-sm break-words">{event.time}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-start">
                                            ...
                                            <div className="overflow-hidden">
                                                <p className="text-[9px] text-gray-400 font-black uppercase tracking-widest mb-0.5">{event.isVirtual ? 'Format' : 'Location'}</p>
                                                <p className="text-gray-900 font-bold text-sm break-words">{event.location}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {event.requiresApplication === false ? (
                                        ...
                                        <div className="w-full py-5 px-8 rounded-2xl bg-red-50 text-red-600 font-black text-center border border-red-100 flex flex-col gap-1">
                                            <span className="uppercase tracking-widest text-xs">Applications Closed</span>
                                            <span className="text-[10px] font-bold opacity-80">
                                                {isCapacityReached ? 'Capacity Reached' : 'Deadline Passed'}
                                            </span>
                                        </div>
                                    ) : (
                                        <Link
